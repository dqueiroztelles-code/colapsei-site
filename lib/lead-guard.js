...
async function enforceRateLimit({ req, supabaseUrl, serviceKey, scope, limit = 8, windowMinutes = 60, fetchImpl = fetch }) {
  ...
  try {
    ...
    console.error('lead_rate_limit_read_fetch_error', scope, error?.message || error);
  ...
  let writeResponse;
  try {
    writeResponse = await fetchImpl(`${supabaseUrl}/rest/v1/submission_rate_limits`, {
      method: 'POST',
      headers: { ...headers, Prefer: 'return=minimal' },
      body: JSON.stringify({ scope, fingerprint })
    });
  } catch (error) {
    console.error('lead_rate_limit_write_fetch_error', scope, error?.message || error);
    return { allowed: true, degraded: true };
  }
